...
namespace Restaurant_Management_System
{
    public partial class categoriesForm : UserControl
    {
                    ...
                    using (SqlCommand checkCat = new SqlCommand(selectCategory, connect))
                    {
                        ...
                        else
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(insertData, connect))
                            {
                                ...
                                MessageBox.Show("Added succesfully!", "Info Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                clearFields();
        ...
        private void categories_uptadeBtn_Click(object sender, EventArgs e)
        {
            if (getID == 0)
            {
            ...
            {
                if (MessageBox.Show($"Are you sure you want to update this ID: {getID} ?", "Confimation"
        ...
        private void categories_deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show($"Are you sure you want to delete this ID: {getID} ?", "Confimation"
                    , MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    using (SqlConnection connect = new SqlConnection(connection))
                    {
