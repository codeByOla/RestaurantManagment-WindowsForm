...
namespace Restaurant_Management_System
{
    public partial class shopForm : UserControl
    {
        ...
        public void loadProducts()
        {
            try
            {
                using (SqlConnection connect = new SqlConnection(connection))
                {
                    ...
                    string selectData = "SELECT * FROM products WHERE status = 'Active'";
                    ...
                    using (SqlCommand cmd = new SqlCommand(selectData, connect))
                    {
                        ...
                        foreach(DataRow row in table.Rows)
                        {
                            ...
                            string price = row["price"] != DBNull.Value ?$" ${ row["price"].ToString()}" : "0.00";
        ...
        private void shop_placeOrderBtn_Click(object sender, EventArgs e)
        {
            if (!check)
            {
                MessageBox.Show("Invalid: Insufficient Amount", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);

            }
            else
            {
                if (MessageBox.Show("Are you sure you want to proceed?", "Confirm Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    using (SqlConnection connect = new SqlConnection(connection))
                    {
                        connect.Open();
                       ...
                       count = Convert.ToInt32(cData.ExecuteScalar())+1;
                        ...
                        string insertData = "INSERT INTO orders (customerId, productids, quantities, prices, total, date_order) VALUES (@cid, @pid,@qty, @total, @price,@date)";
                        ...
                        using (SqlCommand cmd = new SqlCommand(insertData,connect))
                        {
                            ...
                            if(rowAffected > 0)
                            {
                                for(int q = 0; q < productIds.Count; q++)
                                {
                                    ...
                                    if(newStock <0)
                                    {
                                        MessageBox.Show($"Insufficient Stock for product ID: {productIds[q]}", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        return;
                                    }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "OlaFav's Restaurant";
            ...
            string[] header = { "PID", "ProductName", "Qty", "Price" };
